...
export default function router(options) {
		...
		},
		{
			...
			route: (req, res, next) => {
				...
				if (!~valid.indexOf(status)) {
					res.body = { error: `invalid status code, must be one of ${valid.join()}` };
					return next();
				}
				if (count > 0) {
					...
					return res.redirect(status, `/redirect/${status}/${count - 1}${req.query.to ? `?to=${req.query.to}` : ""}`);
				}
	...
	router.use(
		...
		compression({ threshold: 1, filter: () => true }),
