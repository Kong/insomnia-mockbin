...
HTTPConsole.prototype.start = function () {
  ...
  this.router.all('/redirect/:status_code/:count?', function (req, res, next) {
    ...
    var status = parseInt(req.params.status_code);
    ...
    if (count > 0) {
      return res.redirect(status, util.format('http://%s:%s/redirect/%d/%d', req.hostname, self.config.port_mask || self.config.port, status, count - 1));
    }
