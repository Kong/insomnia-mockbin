...
export default function (req, res, next) {
  ...
  req.on('end', function () {
    ...
    var type = contentType ? _parse(contentType) : null
        ...
        dice.on('part', function (part) {
          ...
          part.on('header', function (headers) {
            ...
            if (headers['content-disposition']) {
              var disposition = _parse(headers['content-disposition'][0].replace('form-data', 'form-data/text') || 'form-data/text')
            ...
            if (headers['content-type']) {
              var type = _parse(headers['content-type'][0] || 'application/octet-stream')
