...
module.exports = function (req, res, next) {
  ...
  req.on('end', function () {
    ...
    const type = contentType ? typer.parse(contentType) : null
        ...
        dice.on('part', function (part) {
          ...
          part.on('header', function (headers) {
            ...
            if (headers['content-disposition']) {
              const disposition = typer.parse(headers['content-disposition'][0].replace('form-data', 'form-data/text') || 'form-data/text')
            ...
            if (headers['content-type']) {
              const type = typer.parse(headers['content-type'][0] || 'application/octet-stream')
