...
// @faker-js/faker v10 is ESM-only, but this file is CommonJS.
let faker;
const fakerReady = import("@faker-js/faker").then((mod) => {
	faker = mod.faker;
});
...
module.exports = (client) => async (req, res, next) => {
	...
	const legacyCompoundId = req.params.uuid + req.params[0];

	// TODO: refactor with async/await after upgrading to Express 5
	client.get(`bin:${compoundId}`, function (err, value) {
		try {
			...
			if (value) {
				return onGetMockRoute({ value, client, compoundId, req, res });
			}

			// Value is null, try the legacy compound id for backward compatibility
			if (legacyCompoundId !== compoundId) {
				console.log(
					`Cannot find bin with compoundId ${compoundId}, trying legacy compoundId ${legacyCompoundId}`,
				);
				client.get(`bin:${legacyCompoundId}`, function (err, value) {
					try {
						if (err) {
							debug(err);
							throw err;
						...
						if (value) {
							return onGetMockRoute({
								value,
								client,
								// Use the new compoundId for logging the request, so that the logs are consistent and not fragmented between the new and legacy compound ids.
								compoundId,
								req,
								res,
							});
...
function onGetMockRoute({ value, client, compoundId, req, res }) {
...
(function initEngine() {
	engine = new Liquid({
		parseLimit: 1e6, // 1M, typical size ((character length)) of your templates in each render
		renderLimit: 500, // 500ms, limit the time consumed by each render
		memoryLimit: 1e7, // 10M, memory available for LiquidJS (1e9 for 1GB)
		ownPropertyOnly: true, // Disallow access to prototype properties to prevent prototype pollution attacks
		strictVariables: false, // Undefined variables will be rendered as empty string
		strictFilters: true, // Throw an error when using undefined filters to avoid unintended behavior
		lenientIf: true, // Treat undefined or non-boolean values as falsy in if/unless conditions, avoids render errors
		cache: false, // Disable template caching
	});

	const builtInFilterWhiteList = ["default"];

	function disabledFilter(name) {
		return function () {
			throw new Error(`filter "${name}" unsupported`);
		};
	}
	...
	const disabledTag = {
		parse: function (token) {
			throw new Error(`tag "${token.name}" unsupported`);
		},
	};
...
function renderBody({ template, req }) {
...
const MAX_BODY_CHAR_LENGTH = 1000000; // 1 M
...
class BodyDrop extends Drop {
	...
	liquidMethodMissing(key) {
		...
		if (multipartContetTypes.includes(req.contentType) && req.multiPartSimple) {
...
class HeadersDrop extends Drop {
	constructor(headers) {
		...
		for (const [key, value] of Object.entries(headers)) {
			...
			if (dropHeaders.has(lowerKey)) {
				// Mask the value to prevent leaking it.
				filteredHeaders[key] = undefined;
			} else {
		...
		// use a symbol to avoid name collisions with the headers
		this[headersSymbol] = filteredHeaders;
	}
	liquidMethodMissing(key) {
		...
		if (
			...
			typeof this[headersSymbol][lowercasedKey] === "string"
		) {
			return this[headersSymbol][lowercasedKey];
		}
		return undefined;
	}
}
...
class FakerDrop extends Drop {
	liquidMethodMissing(key) {
		if (
			Object.prototype.hasOwnProperty.call(fakerFunctions, key) &&
			typeof fakerFunctions[key] === "function"
		) {
...
const fakerFunctions = {
	...
	// faker.finance.maskedNumber() removed in v10; replacement per faker's own
	// deprecation notice: https://github.com/faker-js/faker/blob/v9.8.0/src/modules/finance/index.ts#L220
	randomCreditCardMask: () =>
		faker.finance.iban().replace(/(?<=.{4})\w(?=.{2})/g, "*"),
