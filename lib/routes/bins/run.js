...
module.exports = (client) => (req, res, next) => {
	...
	// If method-specific and legacy are the same (e.g. GET), just try once
	if (compoundId === legacyCompoundId) {
	...
	// Try method-specific mock first, then fall back to legacy (all-methods) mock
	client.get(`bin:${compoundId}`, function (mErr, mValue) {
		if (mErr || !mValue) {
			// no method-specific mock found; try the legacy (all-methods) mock
			client.get(`bin:${legacyCompoundId}`, function (err, value) {
				return runMock(err, value, req, res, client, legacyCompoundId, next);
			});
			return;
		}
		return runMock(mErr, mValue, req, res, client, compoundId, next);
	});
};
