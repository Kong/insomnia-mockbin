...
module.exports = (client) => (req, res, next) => {
	...
	status = utils.isValidCompoundId(uuid, path);
	if (status) {
		res.body = {
			error: status.error,
	...
	if (!mockValidate(mock)) {
		...
		try {
			status.message = mockValidate.errors[0].message;
		} catch (e) {
			debug(e);
			status.message =
	...
	validate
		...
		.then(function () {
			...
			res.view = "redirect";
			res.status(200).location(`/bin/${compoundId}`).body = uuid;
		})
		.catch((err) => {
			res.body = {
