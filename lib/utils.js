...
const utils = {
	...
	isValidCompoundId(id, path) {
		let status = null;
		if (!/^mock_[a-f0-9]{32}$/.test(id)) {
			status = {
				error: 'Invalid Mock "id"',
				message: 'The Mock "id" does not match the expected format.',
			};
		}

		// Ensure the `path` length is not over 1900 characters (This will limit the entire path to 2000~).
		if (!status && path.length > 1900) {
			status = {
				error: 'Invalid Mock "path" length',
